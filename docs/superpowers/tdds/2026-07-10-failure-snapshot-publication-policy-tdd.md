...
`notice_push.observability.run_summary_contract` 使用 Pydantic 定义 `RunSummaryContract` 和
`FailureRunSummaryContract`；`notice_push.observability.run_summary` 只负责从领域结果构造并写入合同。
合同采用 `extra="forbid"`、严格基础类型和冻结模型，解析统一使用 `model_validate_json()`，写入统一使用
`model_dump_json()`。正常摘要的 `schema_version` 固定为 `2`：
...
无法通过 `RunSummaryContract` 校验的旧版或损坏摘要不得参与正式发布判断；异常快照构建器会生成带
`fallback=true` 的 `FailureRunSummaryContract`，保留退出码、阻断原因、日志路径和类型化计数，而不是继续传播结构未知的字典。

### 3.4 发布清单 JSON

workflow 在判定为 `blocked` 后、上传 Artifact 前，于临时快照目录生成不可变的 `publication.json`。它记录本次**发布决策与已知的保证结果**，而不伪造尚未执行的 Git/邮件结果：

发布清单由 Pydantic `PublicationManifest` 和嵌套的 `PublicationCounts` 表示。字段使用严格类型，未知字段
直接拒绝；Python 属性 `status`/`blockers` 通过别名稳定序列化为 `publication_status`/
`publication_blockers`。workflow helper 之间只传递该模型或其他专用 Pydantic 结果模型，不使用裸
`json.loads()`、`dict.get()` 解析关键业务 JSON。

唯一例外是 `write_blocked_publication_fallback.py`：它是最终器或项目包无法导入时的应急恢复入口，只
依赖 `argparse`、`json`、`os` 和 `pathlib`。它按固定 schema 生成最小 blocked 清单和完整 Actions 输出；
正常模块恢复可用后，该产物仍必须通过 `PublicationManifest` 校验。此处优先保证故障隔离能力，不复用可能
正处于故障中的业务包。
...
`notices.attachments_json` 与 `notices.assets_json` 不改变既有字节格式：写入继续使用按键稳定排序的标准库
JSON 编码，因为该表示参与 `content_hash`。读取时使用 Pydantic `TypeAdapter` 校验附件和媒体记录；非法
历史值降级为空集合，避免单条旧数据使整次运行失败。这里的标准库编码是哈希兼容边界，不是业务合同解析入口。
...
- 所有新增 JSON 采用 Pydantic 强类型合同和 `schema_version`；不符合当前 run summary 合同的输入进入明确的 failure fallback，不做静默字段补全。
- 生产源 URL、LLM endpoint 和默认模型只允许出现在 `resources/config/runtime.yml`，Python 构造器必须显式接收这些配置。
